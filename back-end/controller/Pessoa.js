...
async function criar(req, res){
    if (!req.body.pessoa) 
        res.status(500).send("Parâmetro Pessoa é obrigatório.");
    else if (!req.body.email)
        res.status(500).send("Parâmetro E-mail é obrigatório.");
    else if (!req.body.telefone)
        res.status(500).send("Parâmetro Telefone é obrigatório.");
    else
        await pessoa
        .create({ 
